...
int main()
{
    ...
    constexpr double frame_duration = 1.0 / 25.0;
    constexpr int substeps_per_frame = 40;
    constexpr double time_step = frame_duration / substeps_per_frame;
    constexpr int frame_count = 251;

    constexpr double Youngs_modulus{4.8e5};
    constexpr double Poisson_ratio{0.2};
    ...
    SolverType type = SolverType::SemiImplicit;

    constexpr double squared_tolerance = 1e-9;
    constexpr int maximum_iterations = 300;
    ...
    for (int frame{1}; frame < frame_count; ++frame)
    {
        ...
        {
            simulation_step(type, particles, colliders, stencils, gravity, time_step, grid_spacing, mu_0, lambda_0, hardening, max_compression, max_stretch, flip_ratio, squared_tolerance, maximum_iterations, beta, timings);
        }
